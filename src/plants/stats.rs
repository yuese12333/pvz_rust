...
mod tests {
    ...
    #[test]
    fn deserialize_targeting_variants() {
        #[derive(Deserialize)]
        struct Wrap {
            #[serde(deserialize_with = "crate::plants::targeting::deserialize_present")]
            targeting: Option<PlantTargeting>,
        }
        let lane: Wrap = from_str("(targeting: LaneForward,)").expect("LaneForward");
        assert_eq!(lane.targeting, Some(PlantTargeting::LaneForward));
        let fwd: Wrap = from_str("(targeting: ForwardRange(2.5),)").expect("ForwardRange");
        assert_eq!(fwd.targeting, Some(PlantTargeting::ForwardRange(2.5)));
    }

    #[test]
    fn validate_accepts_overridden_forward_range() {
        let stats: PlantArchetypeStats = from_str(
            r#"(
            health: 300.0,
            sun_cost: 0,
            cooldown_secs: 7.5,
            targeting: ForwardRange(1.5),
            sprite_dir: "textures/plants/puffshroom",
            idle_frames: 1,
        )"#,
        )
        .expect("minimal puff stats");
        validate_plant_entry(PlantType::PuffShroom, &stats);

        let wrong_variant: PlantArchetypeStats = from_str(
            r#"(
            health: 300.0,
            sun_cost: 0,
            cooldown_secs: 7.5,
        ...
        let err = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            validate_plant_entry(PlantType::PuffShroom, &wrong_variant);
        }));
        assert!(err.is_err());
    }
}
